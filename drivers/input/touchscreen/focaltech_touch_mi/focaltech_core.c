...
static void fts_show_touch_buffer(u8 *buf, int point_num)
{
	...
	count += snprintf(g_sz_debug, PAGE_SIZE, "%02X,%02X,%02X", buf[0],
			  buf[1], buf[2]);
	for (i = 0; i < len; i++) {
		count += snprintf(g_sz_debug + count, PAGE_SIZE, ",%02X",
				  buf[i + 3]);
	}
...
#endif
#if FTS_MT_PROTOCOL_B_EN
	for (finger_count = 0; finger_count < fts_data->pdata->max_touch_number;
	     finger_count++) {
...
static int fts_input_report_b(struct fts_ts_data *data)
{
	...
	for (i = 0; i < data->touch_point; i++) {
		...
		if (EVENT_DOWN(events[i].flag)) {
			...
			if (irq_num == 1)
				FTS_INFO(
					"Report_0x152 resume DOWN report_area %d ",
					fod_overlap_aera);

			touchs |= BIT(events[i].id);
			data->touchs |= BIT(events[i].id);
			if (data->point_id_changed &&
			    data->old_point_id != 0xff &&
			    data->fod_status == 0 &&
			    data->old_point_id != events[i].id) {
				FTS_INFO(
					"%s FOD finger ID is Changed, release old id\n",
					__func__);
...
static int fts_read_and_report_foddata(struct fts_ts_data *data)
{
	...
	z = buf[4];
	XIAOMI_TP_DEBUG(
		"FTS:read fod data: 0x%x    0x%x    0x%x    overlap_area=0x%x\
	...
	if (ret < 0) {
		...
		case 0x24:
			FTS_INFO(
				"DoubleClick Gesture detected, Wakeup panel\n");
		...
		case 0x26:
			if (buf[9] == 0) {
				...
				if (!data->suspended) {
					if (irq_num == 1)
						FTS_INFO(
							"Touch is not in suspend state, report x,y value by touch nomal report\n");
					mutex_unlock(&data->report_mutex);
					return -EINVAL;
				}
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
	...
	if (fts_read_and_report_foddata(data) == 0) {
		...
		ret = fts_i2c_read_reg(fts_data->client, FTS_REG_INT_ACK,
				       &reg_value);
		if (ret < 0)
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
	...
	FTS_INFO("irq in ts_data:%d irq in client:%d", ts_data->irq,
		 ts_data->client->irq);
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
	...
	ts_data->events = (struct ts_event *)kzalloc(
		point_num * sizeof(struct ts_event), GFP_KERNEL);
	if (!ts_data->events) {
...
static void fts_init_touchmode_data(void)
{
	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		FTS_INFO(
			"mode:%d, set cur:%d, get cur:%d, def:%d min:%d max:%d\n",
...
static void fts_update_touchmode_data(int mode)
{
	...
	temp_value =
		(u8)xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE];
	...
	case Touch_Game_Mode:
		...
		if (temp_value == 1) {
			ret = fts_i2c_write_reg(fts_data->client,
						FTS_REG_MONITOR_MODE, 0);
			if (ret < 0)
				FTS_ERROR(
					"disable monitor mode error, ret=%d\n",
					ret);
			fts_data->gamemode_enabled = true;
		} else {
			/*restore touch parameters */
			ret = fts_i2c_write_reg(fts_data->client,
						FTS_REG_MONITOR_MODE, 1);
			if (ret < 0)
				FTS_ERROR(
					"restore monitor mode error, ret=%d\n",
					ret);
			ret = fts_i2c_write_reg(
				fts_data->client, FTS_REG_SENSIVITY,
				(u8)xiaomi_touch_interfaces
			...
			if (ret < 0)
				FTS_ERROR("restore sensitivity error, ret=%d\n",
					  ret);
			ret = fts_i2c_write_reg(
				fts_data->client, FTS_REG_THDIFF,
				(u8)xiaomi_touch_interfaces
					.touch_mode[Touch_Tolerance]
						   [GET_DEF_VALUE]);
			if (ret < 0)
				FTS_ERROR(
					"restore touch smooth error, ret=%d\n",
					ret);
			ret = fts_i2c_write_reg(
				fts_data->client,
				FTS_REG_EDGE_FILTER_ORIENTATION,
				(u8)xiaomi_touch_interfaces
			...
			if (ret < 0)
				FTS_ERROR("restore orientation error, ret=%d\n",
					  ret);
			ret = fts_i2c_write_reg(
				fts_data->client, FTS_REG_EDGE_FILTER_LEVEL,
				(u8)xiaomi_touch_interfaces
	...
	case Touch_Edge_Filter:
		if (fts_data->gamemode_enabled) {
			...
			if (ret < 0)
				FTS_ERROR(
					"write edge filter level error, ret=%d\n",
					ret);
			FTS_ERROR("%s---last_orientation_value=%d\n", __func__,
				  last_orientation_value);
			if (last_orientation_value == 1) {
				ret = fts_i2c_write_reg(
					fts_data->client,
					FTS_REG_EDGE_FILTER_ORIENTATION, 1);
				if (ret < 0)
					FTS_ERROR(
						"write last_orientation_value error, ret=%d\n",
						ret);
			}
			if (last_orientation_value == 3) {
				ret = fts_i2c_write_reg(
					fts_data->client,
					FTS_REG_EDGE_FILTER_ORIENTATION, 2);
				if (ret < 0)
					FTS_ERROR(
						"write last_orientation_value error, ret=%d\n",
						ret);
...
static ssize_t fts_gamemode_test_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	u8 monitor_mode, filter_ori, thdiff, sensitivity, report_rate,
		filter_level;
	...
	fts_i2c_read_reg(fts_data->client, FTS_REG_EDGE_FILTER_ORIENTATION,
			 &filter_ori);
	fts_i2c_read_reg(fts_data->client, FTS_REG_EDGE_FILTER_LEVEL,
			 &filter_level);
	return snprintf(
		buf, PAGE_SIZE,
		"monitor_mode:0x%x\nreport_rate:0x%x\nsensitivity:0x%x\nfollowing performance:0x%x\nedge_filter_orieatation:0x%x\nedge_filter_level:0x%x\n",
		monitor_mode, report_rate, sensitivity, thdiff, filter_ori,
		filter_level);
}
...
#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
static DEVICE_ATTR(gamemode_test, 0644, fts_gamemode_test_show,
		   fts_gamemode_test_store);
#endif

static struct attribute *fts_attrs[] = { &dev_attr_lockdown_info.attr,
#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
					 &dev_attr_gamemode_test.attr,
#endif
					 NULL };

static const struct attribute_group fts_attr_group = { .attrs = fts_attrs };
				      ...
				      size_t count, loff_t *pos)
{
	...
	cnt = snprintf(
		...
		fts_data->lockdown_info[2], fts_data->lockdown_info[3],
		fts_data->lockdown_info[4], fts_data->lockdown_info[5],
		fts_data->lockdown_info[6], fts_data->lockdown_info[7]);
...
static ssize_t fts_tp_debug_read(struct file *file, char __user *buf,
				 size_t count, loff_t *pos)
{
	return 0;
}

ssize_t fts_tp_debug_write(struct file *file, const char __user *buf,
			   size_t count, loff_t *pos)
{
	char tmp[2];
			...
			const struct i2c_device_id *id)
{
	...
	ts_data->tp_lockdown_info_proc = proc_create(
		"tp_lockdown_info", 0644, NULL, &fts_lockdown_info_ops);
	ts_data->tp_fw_version_proc =
		proc_create("tp_fw_version", 0644, NULL, &fts_fw_version_ops);
	ts_data->tp_debug_en_proc =
		proc_create("tp_debug_en", 0644, NULL, &fts_tp_debug_ops);
	...
	tp_debugfs = debugfs_create_file("switch_state", 0660, ts_data->debugfs,
					 ts_data, &tpdbg_operations);
	if (!tp_debugfs) {
	...
	if (ts_data->fts_tp_class == NULL) {
		...
		if (ts_data->fts_tp_class) {
			...
			xiaomi_touch_interfaces.touch_vendor_read =
				fts_get_touch_vendor;
			xiaomi_touch_interfaces.panel_vendor_read =
				fts_panel_vendor_read;
			xiaomi_touch_interfaces.panel_color_read =
				fts_panel_color_read;
			xiaomi_touch_interfaces.panel_display_read =
				fts_panel_display_read;
			xiaomi_touch_interfaces.touch_vendor_read =
				fts_touch_vendor_read;
			xiaomi_touch_interfaces.getModeValue =
				fts_get_mode_value;
			xiaomi_touch_interfaces.setModeValue =
				fts_set_cur_value;
			...
			xiaomi_touch_interfaces.palm_sensor_write =
				fts_palm_sensor_write;
...
*/
	...
	if (fts_gesture_suspend(ts_data->client) == 0) {
		if (!ts_data->aod_status &&
		    (ts_data->fod_status == -1 || ts_data->fod_status == 100)) {
			fts_fod_reg_write(ts_data->client,
					  FTS_REG_GESTURE_FOD_ON, false);
		}
		ts_data->suspended = true;
		ts_data->old_point_id = 0xff;
		ts_data->point_id_changed = false;
		FTS_INFO(
			"%s Tp enter suspend, set old_point_id as default value\n",
			__func__);
		fts_irq_enable();
 ...
 */
	if (ts_data->aod_status ||
	    (ts_data->fod_status != -1 && ts_data->fod_status != 100)) {
		ret = fts_fod_reg_write(ts_data->client, FTS_REG_GESTURE_FOD_ON,
					true);
		if (ret < 0) {
			FTS_ERROR(
				"%s fts_fod_reg_write failed, enter sleep mode!\n",
				__func__);
			goto sleep_mode;
		}

		fts_gesture_reg_write(ts_data->client,
				      FTS_REG_GESTURE_DOUBLETAP_ON, true);
		if (ret < 0) {
			FTS_ERROR(
				"%s fts_gesture_reg_write failed, enter sleep mode!\n",
				__func__);
			goto sleep_mode;
		}
...
static int fts_ts_resume(struct device *dev)
{
	...
	if (fts_data->finger_in_fod) {
		fts_fod_reg_write(ts_data->client, FTS_REG_GESTURE_FOD_NO_CAL,
				  true);
	}
#endif
...
#ifdef CONFIG_TOUCHSCREEN_FTS_FOD
	fts_gesture_reg_write(ts_data->client, FTS_REG_GESTURE_DOUBLETAP_ON,
			      false);
#endif
	ts_data->suspended = false;
	fts_irq_enable();
...
static int fts_pm_suspend(struct device *dev)
{
	...
	if (ts_data->lpwg_mode) {
		...
		if (ret) {
			FTS_INFO("enable_irq_wake(irq:%d) failed",
				 ts_data->irq);
		}
#ifndef CONFIG_TOUCHSCREEN_FTS_FOD
	}
...
static int fts_pm_resume(struct device *dev)
{
	...
	if (ts_data->lpwg_mode) {
		...
		if (ret) {
			FTS_INFO("disable_irq_wake(irq:%d) failed",
				 ts_data->irq);
		}
#ifndef CONFIG_TOUCHSCREEN_FTS_FOD
	}
